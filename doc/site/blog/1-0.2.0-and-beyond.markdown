...
Let's check our goals from [the last blog post](0-hello-fodi.html):
...
Many issues are outdated (or unrelated), and some need a proper response. Some are related to future ideals, things that will take a while to get to. Some are related to the Fodi CLI. It can be difficult to reason about the state of the tasks when they're all over the place, so we've been consolidating.

The good news is the issue list has been drastically reduced, 70+ issues being closed (or resolved). Around 21 of those are marked for future consideration, and 23 moved to the Fodi CLI repository. More consolidation will still continue.
...
For example, discussions of future tasks, ideas or goals [are tagged `future`](https://github.com/fodi-lang/fodi/issues?utf8=%E2%9C%93&q=is%3Aissue+label%3Afuture+). Now we can refer to them later, and re-open them when they become active and relevant again.
...
We've been hammering away on Fodi since 0.1.0 for quite a while. The [change list on GitHub](https://github.com/fodi-lang/fodi/compare/0.1.0...5338275dcdd97fd8d9fc614f420a645500836a59) is too long to display!
...
Thanks to everyone getting involved, every little bit has helped Fodi, no matter how small the contribution.
...
All the code and documentation will still be easy to access in one place, but clarity around Fodi as a project will improve a lot.

The migration has already started, you can [find the fodi-cli repository here](https://github.com/fodi-lang/fodi-cli).
I'm working on some of the refactoring on the [fodi-cli-refactor branch.](https://github.com/fodi-lang/fodi/tree/fodi-cli-refactor)
...
There's also gonna be an additional build target, namely emscripten, so we can easily run Fodi examples on the Fodi website and documentation.
...
There's definitely things in the pipeline though, I've been playing with [adding compound assignments like `+=`](https://github.com/fodi-lang/fodi/pull/701).
...
- Visit the [fodi-lang organization](https://github.com/fodi-lang) on GitHub to get involved.
