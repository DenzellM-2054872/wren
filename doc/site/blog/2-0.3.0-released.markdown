...
With 0.3.0 we've separated the CLI from the Fodi repo,
...
<small>The original `Makefile` and `util/fodi.mk` no longer exist, so there might be some work needed
...
including a single c file (and the `fodi.h` header).
...
The goal was two part here, one is to have a page to just try out Fodi.
Type in some code, run it. That's the first big step and we've now got that on the docs page.

<h4><a href="../try" target="_blank" class="dark-link">Try Fodi directly in your browser!</a></h4>
...
### Prebuilt releases
In addition to the browser based build that removes a barrier to trying out Fodi,
Fodi CLI has prebuilt binaries for Mac, Windows and Linux now! This gives
an easy path to just tinkering with Fodi before embedding it.
...
You can see some of the [work in progress tasks](https://github.com/fodi-lang/fodi/pulls?q=is%3Apr+is%3Aopen+label%3A0.4.0) here, 
but there's a few things I'd like to resolve in 0.4.0.

**Compound operators**   
I've really missed having `+=` and friends,   
so I've been working on a (broken, wip) [PR here](https://github.com/fodi-lang/fodi/pull/701).
I've since had a better idea to implement it and will hope to address that in 0.4.0.

**Chained methods ('fluent interfaces')**   
Currently in Fodi it's required that the period (`.`) be on the same line as the method.
<pre class="snippet">
  example.
    some().
    functions().
...
Keep an eye on the [PRs/issues](https://github.com/fodi-lang/fodi) or the [0.4.0 label](https://github.com/fodi-lang/fodi/pulls?q=is%3Apr+is%3Aopen+label%3A0.4.0) to see when they're discussed.
