...
The [full release notes](https://github.com/fodi-lang/fodi/releases/tag/0.4.0)
...
You can find all the details and the contributions in the [release notes](https://github.com/fodi-lang/fodi/releases/tag/0.4.0).
...
and closing gaps that were left. For example, Fodi supports multi-line strings 
...
Fodi now supports triple quotes for a string `"""`.
...
can be used at runtime, by external tools (and potentially by Fodi itself).
...
All the details for [Attributes can be found here](https://fodi.io/classes.html#attributes).
...
For the API, `fodiSetListElement` now exists, and both set and
`fodiGetListElement` now accept negative indices same as the language side.
