...
With `fodiInterpret()`, we can execute code, but that code can't do anything
...
To make our Fodi code *useful*, the VM needs to communicate with the outside
world. Fodi uses a single unified set of functions for passing data into and out
of the VM. These functions are based on two fundamental concepts: **slots** and
**handles**.

## The Slot Array

When you want to send data to Fodi, read data from it, or generally monkey
around with Fodi objects from C, you do so by going through an array of slots.
...
fodiEnsureSlots(FodiVM* vm, int slotCount);
</pre>
...
before populating the slots with data that you want to send to Fodi.
...
you pass control back to Fodi. That includes calling `fodiCall()` or
`fodiInterpret()`, or returning from a [foreign method][].

[foreign method]: calling-c-from-fodi.html

If you read or write from a slot that you haven't ensured is valid, Fodi makes
...
the size you've ensured. Fodi reuses the memory for this array when possible,
so you may get one bigger than you need if it happened to be laying around.

When Fodi [calls your C code][] and passes data to you, it ensures there are
enough slots for the objects it is sending you.

[calls your c code]: calling-c-from-fodi.html

### Writing slots

Once you have some slots, you store data in them using a number of functions all
named `fodiSetSlot<type>()` where `<type>` is the kind of data. We'll start with
the simple ones:
...
[Fodi value][]. (Since Fodi's [native number type][] *is* a double, there's not
really much *conversion* going on, but you get the idea.)

[fodi value]: ../values.html
[native number type]: ../values.html#numbers

You can also pass string data to Fodi:

<pre class="snippet" data-lang="c">
void fodiSetSlotBytes(FodiVM* vm, int slot,
                      const char* bytes, size_t length);

void fodiSetSlotString(FodiVM* vm, int slot,
                       const char* text);
</pre>

Both of these copy the bytes into a new [String][] object managed by Fodi's
garbage collector, so you can free your copy of it after you call this. The
difference between the two is that `fodiSetSlotBytes()` takes an explicit
length. Since Fodi strings may contain arbitrary byte values, including the null
...
These take a Fodi value of the corresponding type and convert it to its raw C
representation. For strings, we have:

<pre class="snippet" data-lang="c">
const char* fodiGetSlotString(FodiVM* vm, int slot);
const char* fodiGetSlotBytes(FodiVM* vm, int slot,
                             int* length);
</pre>
...
these return a direct pointer to the bytes managed by Fodi. You should not hold
on to this pointer for long. Fodi does not promise that it won't move or free
the data.

With these functions, you are going from dynamically typed Fodi data to
...
instances of classes come back as `FODI_TYPE_UNKNOWN`. If you want to move that
kind of data between Fodi and C, you'll have to pull the object apart into
...
<pre class="snippet" data-lang="c">
void fodiGetVariable(FodiVM* vm, const char* module,
...
The slot array is fine for moving a fixed number of objects between Fodi and
...
slot&mdash;either one you created from C or from Fodi&mdash;you can add elements
to it using:

<pre class="snippet" data-lang="c">
void fodiInsertInList(FodiVM* vm, int listSlot, int index,
                      int elementSlot);
</pre>
...
* `listSlot` is the slot where the list object is stored. That's the list you'll
  ...
  to `fodiSetSlotNewList()`.

* `index` is the index within the list where you want to insert the element.
  Just like from within Fodi, you can use a negative number to count back from
...
`fodiInsertInListDouble()`, `fodiInsertInListBool()`, etc.

## Handles

Slots are pretty good for shuttling primitive data between C and Fodi, but they
have two limitations:

1. **They are short-lived.** As soon as you execute some more Fodi code, the
...
This takes the object stored in the given slot, creates a new FodiHandle to wrap
it, and returns a pointer to it back to you. You can send that wrapped object
back to Fodi by calling:

<pre class="snippet" data-lang="c">
void fodiSetSlotHandle(FodiVM* vm, int slot, FodiHandle* handle);
</pre>

Note that this doesn't invalidate your FodiHandle. You can still keep using it.

### Retaining and releasing handles

A handle is an opaque wrapper around an object of any type, but just as
important, it's a *persistent* one. When Fodi gives you a pointer to a
FodiHandle, it guarantees that that pointer remains valid. You can keep it
around as long as you want. Even if a garbage collection occurs, Fodi will
ensure the handle and the object it wraps are kept safely in memory.

Internally, Fodi keeps a list of all of the FodiHandles that have been created.
...
Since C relies on manual memory management, FodiHandle does too. When you are
...
FodiHandle wrapper itself. After you call this, you cannot use that pointer
again.

You must release every FodiHandle you've created before shutting down the VM.
Fodi warns you if you don't, since it implies you've probably leaked a resource
somewhere.

Now we know how to pass values between Fodi and C, but we don't know how to
actually *do* anything with them. Next, we'll learn how to use slots to pass
parameters to a Fodi method from C...

<a class="right" href="calling-fodi-from-c.html">Calling Fodi from C &rarr;</a>
