...
If you'd like to try Fodi, you have a few options.

 *  **In your browser.** You can try Fodi **[right here](./try/)**!
 *  **On your computer.** The [Fodi CLI](cli) project is a downloadable executable
      to run scripts with access to file io and more. See the [Fodi CLI docs](cli).
 * **Embedded in your code.** See how to [build and embed Fodi](#embed-the-vm) below.   
...
**The Fodi Virtual Machine** is the core of the language that executes Fodi
...
To build the Fodi library, we look inside the `projects/` folder.
In here you'll find ready to go projects for `Visual Studio`, `XCode` and tools like `make`.

 * **Windows** Open `fodi.sln` inside `projects/vs2019/` (or `vs2017`), hit build.
 * **Mac** Open `fodi.xcworkspace` inside `projects/xcode/`, hit build.
...
* **Static Linking** `fodi.lib` on Windows, `libfodi.a` elsewhere.
* **Dynamic Linking** `fodi.dll` on Windows, `libfodi.so` on Linux, and `libfodi.dylib` on Mac.

<small>
  Note that the default build will also generate `fodi_test` inside of `bin/`,   
  a binary that is used to run the language tests. It can execute simple scripts.
</small>

**Other platforms**   
If your platform isn't explicitly supported,
it is recommended that you include the Fodi source
in your project for a portable experience.

### Including the code in your project

**all source files**   
The alternative to building via the provided projects is to include the fodi source code in your project.
...
This is _all of the fodi source code in one file_.

This file can be generated by running `python3 util/generate_amalgamation.py > build/fodi.c`, 
which saves the generated output in `build/fodi.c`.

Include `build/fodi.c` and `src/include/fodi.h` in your project code and you're good to go.
 ...
 *  Ask on the [Fodi mailing list][list] (which is pretty quiet).
...
[repo]: https://github.com/fodi-lang/fodi
[issue_cli]: https://github.com/fodi-lang/fodi-cli/issues
[repo_cli]: https://github.com/fodi-lang/fodi-cli
