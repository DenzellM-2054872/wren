...
ALL_LDFLAGS += $(LDFLAGS) -L/usr/lib64 -m64 -shared -Wl,-soname=libfodi.so -s
...
ALL_LDFLAGS += $(LDFLAGS) -L/usr/lib32 -m32 -shared -Wl,-soname=libfodi.so -s
...
ALL_LDFLAGS += $(LDFLAGS) -shared -Wl,-soname=libfodi.so -s

else ifeq ($(config),debug_64bit)
TARGETDIR = ../../lib
TARGET = $(TARGETDIR)/libfodi_d.so
...
ALL_LDFLAGS += $(LDFLAGS) -L/usr/lib64 -m64 -shared -Wl,-soname=libfodi_d.so
...
ALL_LDFLAGS += $(LDFLAGS) -L/usr/lib32 -m32 -shared -Wl,-soname=libfodi_d.so
...
ALL_LDFLAGS += $(LDFLAGS) -shared -Wl,-soname=libfodi_d.so
...
OBJECTS += $(OBJDIR)/fodi_instructions.o
...
$(OBJDIR)/fodi_debug.o: ../../src/vm/fodi_debug.c
	@echo $(notdir $<)
	$(SILENT) $(CC) $(ALL_CFLAGS) $(FORCE_INCLUDE) -o "$@" -MF "$(@:%.o=%.d)" -c "$<"
$(OBJDIR)/fodi_instructions.o: ../../src/vm/fodi_instructions.c
	@echo $(notdir $<)
	$(SILENT) $(CC) $(ALL_CFLAGS) $(FORCE_INCLUDE) -o "$@" -MF "$(@:%.o=%.d)" -c "$<"
