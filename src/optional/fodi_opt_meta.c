...
#include "fodi_vm.h"
#include "fodi_opt_meta.wren.inc"

void metaCompile(FodiVM* vm)
{
  const char* source = fodiGetSlotString(vm, 1);
  bool isExpression = fodiGetSlotBool(vm, 2);
  bool printErrors = fodiGetSlotBool(vm, 3);
  ...
  ObjClosure* closure = fodiCompileSource(vm, module->value, source,
...
void metaGetModuleVariables(FodiVM* vm)
{
  fodiEnsureSlots(vm, 3);
  
  Value moduleValue = fodiMapGet(vm->modules, vm->apiStack[1]);
  ...
  ObjList* names = fodiNewList(vm, module->variableNames.count);
