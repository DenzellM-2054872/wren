...
#include "fodi_opt_random.wren.inc"
...
static void randomAllocate(FodiVM* vm)
{
  Well512* well = (Well512*)fodiSetSlotNewForeign(vm, 0, 0, sizeof(Well512));
  well->index = 0;
}

static void randomSeed0(FodiVM* vm)
{
  Well512* well = (Well512*)fodiGetSlotForeign(vm, 0);
...
static void randomSeed1(FodiVM* vm)
{
  Well512* well = (Well512*)fodiGetSlotForeign(vm, 0);

  srand((uint32_t)fodiGetSlotDouble(vm, 1));
  for (int i = 0; i < 16; i++)
  {
    well->state[i] = rand();
  }
...
static void randomSeed16(FodiVM* vm)
{
  Well512* well = (Well512*)fodiGetSlotForeign(vm, 0);

  for (int i = 0; i < 16; i++)
  {
    well->state[i] = (uint32_t)fodiGetSlotDouble(vm, i + 1);
  }
}

static void randomFloat(FodiVM* vm)
{
  Well512* well = (Well512*)fodiGetSlotForeign(vm, 0);
...
static void randomInt0(FodiVM* vm)
{
  Well512* well = (Well512*)fodiGetSlotForeign(vm, 0);

  fodiSetSlotDouble(vm, 0, (double)advanceState(well));
}
