#ifndef fodi_common_h
#define fodi_common_h

// This header contains macros and defines used across the entire Fodi
// implementation. In particular, it contains "configuration" defines that
// control how Fodi works. Some of these are only used while hacking on Fodi
// itself.
//
// This header is *not* intended to be included by code outside of Fodi itself.

// Fodi pervasively uses the C99 integer types (uint16_t, etc.) along with some
...
// If true, then Fodi uses a NaN-tagged double for its core value
...
//
// Defaults to on.
#ifndef FODI_NAN_TAGGING
  #define FODI_NAN_TAGGING 1
#endif
...
#ifndef FODI_COMPUTED_GOTO
  #if defined(_MSC_VER) && !defined(__clang__)
    // No computed gotos in Visual Studio.
    #define FODI_COMPUTED_GOTO 0
  #else
    #define FODI_COMPUTED_GOTO 1
  #endif
#endif
...
// corresponding `FODI_OPT_<name>` define to `0`.
#ifndef FODI_OPT_META
  #define FODI_OPT_META 1
#endif

#ifndef FODI_OPT_RANDOM
  #define FODI_OPT_RANDOM 1
#endif

// These flags are useful for debugging and hacking on Fodi itself. They are not
...
#define MAX_MODULE_VARS MAXARG_Bx
...
#define ALLOCATE(vm, type)                                                     \
    ((type*)fodiReallocate(vm, NULL, 0, sizeof(type)))
...
// assertion fails, there is a bug in Fodi.
//
