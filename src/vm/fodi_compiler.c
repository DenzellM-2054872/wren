...
#if FODI_DEBUG_DUMP_COMPILED_CODE
...
typedef struct
{
  ...
  int reg; // The register this local variable is stored in
                       ...
                       const char *format, va_list args)
{
  ...
  // Only report errors if there is a FodiErrorFn to handle them.
  ...
  parser->vm->config.errorFn(parser->vm, FODI_ERROR_COMPILE,
...
static void error(Compiler *compiler, const char *format, ...)
{
  ...
  if (compiler->fn != NULL)
  {
    fodiDumpConstants(compiler->fn);
    fodiDumpCode(compiler->parser->vm, compiler->fn, -1);
  }
#endif
...
// Reserves a register and returns its index.
...
// Returns the index of a temporary register.
static int tempRegister(Compiler *compiler)
{
  return compiler->freeRegister;
}
...
static int addConstant(Compiler *compiler, Value constant)
{
  ...
  {
    Value existing = fodiMapGet(compiler->constants, constant);
    if (IS_NUM(existing))
  ...
  {
    if (IS_OBJ(constant))
      fodiPushRoot(compiler->parser->vm, AS_OBJ(constant));
    fodiValueBufferWrite(compiler->parser->vm, &compiler->fn->constants,
                         constant);
    if (IS_OBJ(constant))
      fodiPopRoot(compiler->parser->vm);

    if (compiler->constants == NULL)
    {
      compiler->constants = fodiNewMap(compiler->parser->vm);
    }
    fodiMapSet(compiler->parser->vm, compiler->constants, constant,
                         ...
                         bool isMethod)
{
  ...
  compiler->attributes = fodiNewMap(parser->vm);
  compiler->fn = fodiNewFunction(parser->vm, parser->module,
                                 compiler->numLocals);
}
...
{
  ByteBuffer string;
  fodiByteBufferInit(&string);
  fodiByteBufferWrite(parser->vm, &string, firstChar);

  while (isName(peekChar(parser)) || isDigit(peekChar(parser)))
  {
    char c = nextChar(parser);
    fodiByteBufferWrite(parser->vm, &string, c);
  }
  ...
  parser->next.value = fodiNewStringLength(parser->vm,
                                           (char *)string.data, string.count);

  fodiByteBufferClear(parser->vm, &string);
...
static void readUnicodeEscape(Parser *parser, ByteBuffer *string, int length)
{
  ...
  int numBytes = fodiUtf8EncodeNumBytes(value);
  if (numBytes != 0)
  {
    fodiByteBufferFill(parser->vm, string, 0, numBytes);
    fodiUtf8Encode(value, string->data + string->count - numBytes);
  }
}
...
static void readString(Parser *parser)
{
  ...
  for (;;)
  {
    ...
    if (c == '\\')
    {
      ...
      {
      case '"':
        fodiByteBufferWrite(parser->vm, &string, '"');
        break;
      case '\\':
        fodiByteBufferWrite(parser->vm, &string, '\\');
        break;
      case '%':
        ...
        break;
      case '0':
        fodiByteBufferWrite(parser->vm, &string, '\0');
        break;
      case 'a':
        fodiByteBufferWrite(parser->vm, &string, '\a');
        break;
      case 'b':
        fodiByteBufferWrite(parser->vm, &string, '\b');
        break;
      case 'e':
        fodiByteBufferWrite(parser->vm, &string, '\33');
        break;
      case 'f':
        fodiByteBufferWrite(parser->vm, &string, '\f');
        break;
      case 'n':
        fodiByteBufferWrite(parser->vm, &string, '\n');
        break;
      case 'r':
        fodiByteBufferWrite(parser->vm, &string, '\r');
        break;
      case 't':
        fodiByteBufferWrite(parser->vm, &string, '\t');
        break;
      ...
      case 'U':
        ...
        break;
      case 'v':
        fodiByteBufferWrite(parser->vm, &string, '\v');
        break;
      case 'x':
        fodiByteBufferWrite(parser->vm, &string,
    ...
    default:
      ...
      else
      {
        ...
        else
        {
          ...
          // meaningful code units in Fodi, the lexer works on raw bytes,
...
static int emitInstruction(Compiler *compiler, Instruction instruction)
{
  ...
  fodiIntBufferWrite(compiler->parser->vm, &compiler->fn->debug->sourceLines,
                     compiler->parser->previous.line);

  // fodiDumpInstruction(compiler->parser->vm, compiler->fn, compiler->fn->code.count - 1);
...
static void emitReturnInstruction(Compiler *compiler, int retReg)
{
  ...
  {
    Instruction last = compiler->fn->code.data[compiler->fn->code.count - 1];
    // if the last instruction is already a return, don't emit another
    if (GET_OPCODE(last) == OP_RETURN)
      return;
  }

  if (retReg == -1)
  {
    emitInstruction(compiler, makeInstructionABC(OP_RETURN, 0, 0, 0, 0));
  }
  else
  {
    emitInstruction(compiler, makeInstructionABC(OP_RETURN, retReg, 1, 0, 0));
  }
}
...
void emitInfixBoolOpcall(Compiler * compiler, Code op, Code opk, bool invert, ReturnValue *left, ReturnValue *right){
  ...
  else if (right->type == RET_CONST_INDEX)
    emitInstruction(compiler, makeInstructionABC(opk, (int) invert, AS_NUM(left->value), AS_NUM(right->value), 0));
  else
    emitInstruction(compiler, makeInstructionABC(op, (int) invert, AS_NUM(left->value), AS_NUM(right->value), 0));
}

void emitInfixOpcall(Compiler * compiler, Code op, Code opk, int dest, ReturnValue *left, ReturnValue *right){
  if (left->type == RET_CONST_INDEX)
    emitInstruction(compiler, makeInstructionABC(opk, dest, AS_NUM(right->value), AS_NUM(left->value), 1));
  else if (right->type == RET_CONST_INDEX)
    emitInstruction(compiler, makeInstructionABC(opk, dest, AS_NUM(left->value), AS_NUM(right->value), 0));
  else
...
static void insertValue(Compiler *compiler, ReturnValue *ret, bool lock, bool allowConst)
{
  switch (ret->type)
  {
    case RET_RETURN:
    case RET_REG:
      if(lock && AS_NUM(ret->value) == tempRegister(compiler)) reserveRegister(compiler);
...
{
  int index;
  if(ret->type == RET_CONST && (index = addConstant(compiler, ret->value)) <= UINT8_MAX)
  {
    emitInstruction(compiler, makeInstructionABC(OP_ADDELEMK, tempRegister(compiler), listReg, index, 1));
  }
  else
  {
...
static int addLocal(Compiler *compiler, const char *name, int length)
{
  ...
  local->reg = reserveRegister(compiler);
...
static int declareVariable(Compiler *compiler, Token *token)
{
  ...
  {
    int line = -1;
    int symbol = fodiDefineVariable(compiler->parser->vm,
...
static void loadValue(Compiler *compiler, ReturnValue *ret)
{
  switch (ret->type)
  {
  case RET_CONST:
...
static void defineVariable(Compiler *compiler, int symbol, ReturnValue *ret)
{
  ...
  emitInstruction(compiler, makeInstructionABx(OP_SETGLOBAL, AS_NUM(ret->value), symbol));
}
  ...
  while (local >= 0 && compiler->locals[local].depth >= depth)
  {
    ...
    if (compiler->locals[local].isUpvalue)
    {
      emitInstruction(compiler, makeInstructionABC(OP_CLOSE,
                                                   compiler->locals[local].reg, 0, 0, 0));
    }
    local--;
  }
...
static Variable resolveName(Compiler *compiler, const char *name, int length)
{
  ...
  variable.index = fodiSymbolTableFind(&compiler->parser->module->variableNames,
                                       name, length);
  return variable;
}
                          ...
                          const char *debugName, int debugNameLength)
{
  ...
  fodiFunctionBindName(compiler->parser->vm, compiler->fn,
                       debugName, debugNameLength);
  int Kproto = -1;
  // In the function that contains this one, load the resulting function object.
  if (compiler->parent != NULL)
  {
    ObjClosure *closure = fodiNewClosure(compiler->parser->vm, compiler->fn, true);
    ...
    for (int i = 0; i < compiler->fn->numUpvalues; i++)
    {
      closure->protoUpvalues[i] = fodiNewProtoUpvalue(compiler->parser->vm,
...
#if FODI_DEBUG_DUMP_COMPILED_CODE
  fodiDumpConstants(compiler->fn);
  fodiDumpCode(compiler->parser->vm, compiler->fn, Kproto);
#endif
...
typedef void (*GrammarFn)(Compiler *, bool canAssign, ReturnValue *ret);
...
static void expression(Compiler *compiler, ReturnValue *ret);
static void statement(Compiler *compiler);
static void definition(Compiler *compiler);
static void parsePrecedence(Compiler *compiler, Precedence precedence, ReturnValue *ret);
...
static void patchJump(Compiler *compiler, int offset)
{
  ...
  int jump = compiler->fn->code.count - offset - 1;
  if (abs(jump) > MAX_JUMP)
    error(compiler, "Too much code to jump over.");
  setInstructionField(&compiler->fn->code.data[offset], Field_sJx, jump);
}
...
{
  ReturnValue ret = {RET_NONE, NULL_VAL};
...
{
  return fodiSymbolTableEnsure(compiler->parser->vm,
                          ...
                          Signature *signature, ReturnValue *ret)
{
  ...
  *ret = REG_RETURN_REG(compiler->freeRegister);
}
...
static OpcallType opCallSymbol(Compiler *compiler, Signature *signature)
{
  ...
  if (strncmp(signature->name, "add", signature->length) == 0 &&
      signature->arity == 1 && signature->length == 3)
    return OPCALL_ADD;
...
static int estimateArity(Compiler* compiler, Signature *signature)
{
  ...
  ObjString *callName = AS_STRING(fodiNewStringLength(compiler->parser->vm, signature->name, rParen - signature->name + 1));
  int args = 0;
  bool newArg = true;

  if(signature->length == callName->length)
    return 0;
...
static void opCall(Compiler *compiler, OpcallType opcall, ReturnValue *ret)
{
  ...
  int index;
  if (arg.type == RET_CONST && (index = addConstant(compiler, arg.value)) <= UINT8_MAX){
    constArg = true;
  }
  else 
  ...
  {
    case OPCALL_ADD:
      emitInstruction(compiler, makeInstructionABC(constArg ? OP_ADDELEMK : OP_ADDELEM, startReg, AS_NUM(ret->value), constArg ? index : AS_NUM(arg.value), 1));
      break; 
    case OPCALL_ITERATE:
      emitInstruction(compiler, makeInstructionABC(OP_ITERATE, startReg, AS_NUM(ret->value), constArg ? index : AS_NUM(arg.value), constArg ? 1 : 0));
      break; 
    case OPCALL_ITERATORVALUE:
      emitInstruction(compiler, makeInstructionABC(OP_ITERATORVALUE, startReg, AS_NUM(ret->value), constArg ? index : AS_NUM(arg.value), constArg ? 1 : 0));
      break; 
                       ...
                       Signature *signature, ReturnValue *ret)
{
  ...
  if (match(compiler, TOKEN_LEFT_PAREN))
  {
    ...
    {
      finishArgumentList(compiler, &called, AS_NUM(ret->value));
    }
    consume(compiler, TOKEN_RIGHT_PAREN, "Expect ')' after arguments.");
  }
...
{
  int symbol = fodiSymbolTableFind(&compiler->parser->module->variableNames,
                                   name, strlen(name));
...
static void list(Compiler *compiler, bool canAssign, ReturnValue *ret)
{
  ...
  ObjList* listObj = fodiNewList(compiler->parser->vm, 0);
  ReturnValue listRet = REG_RETURN_CONST(NULL_VAL);
  ...
  do
  {
    ...
    {
      emitInstruction(compiler, makeInstructionABx(OP_LOADK, listReg, addConstant(compiler, OBJ_VAL(listObj))));
  ...
  if (listRet.type == RET_CONST)
    *ret = REG_RETURN_CONST(OBJ_VAL(listObj));
  else
...
static void map(Compiler *compiler, bool canAssign, ReturnValue *ret)
{
  ...
  ObjMap* mapObj = fodiNewMap(compiler->parser->vm);
  ReturnValue mapRet = REG_RETURN_CONST(NULL_VAL);
  ...
  do
  {
    ...
    if((keyRet.type == RET_REG && AS_NUM(keyRet.value) == -1) || (keyRet.type == RET_REG && AS_NUM(valueRet.value) == -1))
    continue;
    ...
    if(mapRet.type == RET_CONST && keyRet.type == RET_CONST && valueRet.type == RET_CONST)
    {
      ...
      fodiMapSet(compiler->parser->vm, mapObj, keyRet.value, valueRet.value);
      continue;
    }
    ...
    else if (mapRet.type == RET_CONST)
    {
      ...
      emitInstruction(compiler, makeInstructionABx(OP_LOADK, mapStart, addConstant(compiler, OBJ_VAL(mapObj))));
...
static bool foldUnaryOp(Compiler *compiler, ReturnValue *ret, UnarySymbol symbol)
{
  ...
  case METHOD_UNARY_NOT:
    result = fodiNot(compiler->parser->vm, ret->value);
    break;
  case METHOD_UNARY_NEG:
    result = fodiNegative(compiler->parser->vm, ret->value);
    break;
...
static void unaryOp(Compiler *compiler, bool canAssign, ReturnValue *ret)
{
  ...
  if (unaryOpCode(compiler, canAssign, ret, rule))
    return;
  ignoreNewlines(compiler);
  int startRegister = tempRegister(compiler);
...
static void handleField(Compiler *compiler, bool isLoad, int receiverReg, int field, ReturnValue *ret)
{
  if (isLoad)
  {
    emitInstruction(compiler, makeInstructionABC(OP_GETFIELD, tempRegister(compiler), receiverReg, field, 0));
    *ret = REG_RETURN_REG(tempRegister(compiler));
  }
  else
  {
    emitInstruction(compiler, makeInstructionABC(OP_SETFIELD, AS_NUM(ret->value), receiverReg, field, 0));
...
static void field(Compiler *compiler, bool canAssign, ReturnValue *ret)
{
  ...
  else
  {
    ...
    field = fodiSymbolTableEnsure(compiler->parser->vm, &enclosingClass->fields,
  ...
  if (compiler->parent == NULL || compiler->parent->enclosingClass != enclosingClass)
  {
    ...
    handleField(compiler, isLoad, AS_NUM(thisRet.value), field, ret);
  }
  else
  {
...
static void bareName(Compiler *compiler, bool canAssign, Variable variable, ReturnValue *ret)
{
  ...
  if (canAssign && match(compiler, TOKEN_EQ))
  {
    ...
    {
    case SCOPE_LOCAL:
      assignValue(compiler, ret, compiler->locals[variable.index].reg);
      break;

    case SCOPE_UPVALUE:
      insertValue(compiler, ret, false, false);
      emitInstruction(compiler, makeInstructionABx(OP_SETUPVAL, AS_NUM(ret->value), variable.index));
      break;

    case SCOPE_MODULE:
      insertValue(compiler, ret, false, false);
      emitInstruction(compiler, makeInstructionABx(OP_SETGLOBAL, AS_NUM(ret->value), variable.index));
      break;
  ...
  loadVariable(compiler, variable, ret);

  allowLineBeforeDot(compiler);
}

static void staticField(Compiler *compiler, bool canAssign, ReturnValue *ret)
{
  ...
  if (resolveLocal(classCompiler, token->start, token->length) == -1)
  {
    ...
    emitInstruction(classCompiler,
                    makeInstructionABC(OP_LOADNULL, tempRegister(classCompiler), 0, 0, 0));
    *ret = REG_RETURN_RETURN(tempRegister(classCompiler));
...
static void name(Compiler *compiler, bool canAssign, ReturnValue *ret)
{
  ...
  // on this.
  if (fodiIsLocalName(token->start) && getEnclosingClass(compiler) != NULL)
  {
  ...
  if (variable.index == -1)
  {
    ...
    variable.index = fodiDeclareVariable(compiler->parser->vm,
                                         compiler->parser->module,
                                         token->start, token->length,
                                         token->line);
...
static int estimateSubscriptArity(Compiler* compiler, Signature *signature)
{
  ...
  ObjString *callName = AS_STRING(fodiNewStringLength(compiler->parser->vm, compiler->parser->tokenStart, rParen - compiler->parser->tokenStart + 1));
  int args = 0;
  bool newArg = true;
  for (size_t i = signature->length; i < callName->length; i++)
  {
...
static void opSubscript(Compiler *compiler, bool canAssign, ReturnValue *ret)
{
  ...
  constArg = arg.type == RET_CONST_INDEX ;
  ...
  if (canAssign && match(compiler, TOKEN_EQ))
  {
    ...
    emitInstruction(compiler,
                makeInstructionABC(OP_SETSUB, AS_NUM(val.value), AS_NUM(ret->value), AS_NUM(arg.value), constArg ? 1 : 0));
    compiler->freeRegister = startReg;
    *ret = REG_RETURN_REG(AS_NUM(val.value));
    return;
  }
  emitInstruction(compiler,
...
static void subscript(Compiler *compiler, bool canAssign, ReturnValue *ret)
{
  ...
  // Parse the argument list.
  finishArgumentList(compiler, &signature, AS_NUM(ret->value));
  consume(compiler, TOKEN_RIGHT_BRACKET, "Expect ']' after arguments.");
...
static int emitIfJump(Compiler *compiler, ReturnValue *ret, int offset, bool cond)
{
  ...
  {
    emitInstruction(compiler,
                    makeInstructionABx(OP_LOADK, tempRegister(compiler), addConstant(compiler, ret->value)));
    *ret = REG_RETURN_REG(tempRegister(compiler));
  }
  if (ret->type == RET_REG || ret->type == RET_RETURN)
    emitInstruction(compiler,
                    makeInstructionABC(OP_TEST, AS_NUM(ret->value), AS_NUM(ret->value), (int)cond, 0));
...
static void or_(Compiler *compiler, bool canAssign, ReturnValue *ret)
{
  ...
  int jump = emitIfJump(compiler, ret, 0, false);
...
static void conditional(Compiler *compiler, bool canAssign, ReturnValue *ret)
{
  ...
  int ifJump = emitIfJump(compiler, ret, 0, true);
  // Compile the then branch.
  int condRegister = tempRegister(compiler);
  ...
  int elseJump = emitJump(compiler, 0);
...
static void constFolding(Compiler *compiler, ReturnValue *left, ReturnValue *right, InfixSymbol symbol, ReturnValue *ret)
{
  ...
  {
  case SIG_METHOD_EQ:
    result = BOOL_VAL(fodiValuesEqual(left->value, right->value));
    break;
  case SIG_METHOD_NEQ:
    result = BOOL_VAL(!fodiValuesEqual(left->value, right->value));
    break;
  case SIG_METHOD_LT:
    result = fodiValuesLess(compiler->parser->vm, left->value, right->value, false);
    break;
  case SIG_METHOD_GT:
    result = fodiValuesLessEq(compiler->parser->vm, left->value, right->value, true);
    break;
  case SIG_METHOD_LTEQ:
    result = fodiValuesLessEq(compiler->parser->vm, left->value, right->value, false);
    break;
  case SIG_METHOD_GTEQ:
    result = fodiValuesLess(compiler->parser->vm, left->value, right->value, true);
    break;
  case SIG_METHOD_ADD:
    result = fodiAdd(compiler->parser->vm, left->value, right->value);
    break;
  case SIG_METHOD_SUB:
    result = fodiSubtract(compiler->parser->vm, left->value, right->value);
    break;
  case SIG_METHOD_MUL:
    result = fodiMultiply(compiler->parser->vm, left->value, right->value);
    break;
  case SIG_METHOD_DIV:
    result = fodiDivide(compiler->parser->vm, left->value, right->value);
    break;
  case SIG_METHOD_EXCRANGE:
  case SIG_METHOD_INCRANGE:

    if(!IS_NUM(left->value)){
      error(compiler, "Left hand side of range must be a number.");
      return;
    }

    if (!IS_NUM(right->value)){
      error(compiler, "Right hand side of range must be a number.");
      return;
    }

    result = fodiNewRange(compiler->parser->vm, AS_NUM(left->value), AS_NUM(right->value), symbol == SIG_METHOD_INCRANGE);
    break;
  ...
  if (compiler->parser->vm->fiber != NULL && fodiHasError(compiler->parser->vm->fiber))
...
static bool infixOpCode(Compiler *compiler, bool canAssign, ReturnValue *ret, GrammarRule *rule)
{
  ...
  case SIG_METHOD_EXCRANGE:
  case SIG_METHOD_INCRANGE:
    if (ret->type == RET_CONST_INDEX){
      emitInstruction(compiler, makeInstructionABx(OP_LOADK, tempRegister(compiler), AS_NUM(ret->value)));
      emitInstruction(compiler, makeInstructionABC(OP_RANGE, startRegister, tempRegister(compiler), AS_NUM(right.value), symbol == SIG_METHOD_INCRANGE ? 1 : 0));
    }
    else if (right.type == RET_CONST_INDEX){
      emitInstruction(compiler, makeInstructionABx(OP_LOADK, tempRegister(compiler), AS_NUM(right.value)));
      emitInstruction(compiler, makeInstructionABC(OP_RANGE, startRegister, AS_NUM(ret->value), tempRegister(compiler), symbol == SIG_METHOD_INCRANGE ? 1 : 0));
    }
    else
      emitInstruction(compiler, makeInstructionABC(OP_RANGE, startRegister, AS_NUM(ret->value), AS_NUM(right.value), symbol == SIG_METHOD_INCRANGE ? 1 : 0));
    *ret = REG_RETURN_REG(startRegister);
    break;
  default:
    UNREACHABLE();
...
void infixOp(Compiler *compiler, bool canAssign, ReturnValue *ret)
{
  ...
  insertTarget(&compiler->fn->code, startRegister);
  *ret = REG_RETURN_REG(startRegister);
...
void parsePrecedence(Compiler *compiler, Precedence precedence, ReturnValue *ret)
{
  nextToken(compiler->parser);
...
{
  compiler->loop->exitJump = emitIfJump(compiler, ret, 0, true);
}
...
static void endLoop(Compiler *compiler)
{
  ...
  int regLoopOffset = -(compiler->fn->code.count - compiler->loop->start);
  ...
  while (i < compiler->fn->code.count)
  {
    ...
    if (compiler->fn->code.data[i] == nopJump)
    {
...
static void forStatement(Compiler *compiler)
{
  ...
  // actually get names that aren't valid Fodi identfiers, but that's the basic
  // idea.
  //
  ...
  expression(compiler, &ret);
  int seqSlot = compiler->locals[addLocal(compiler, "seq ", 4)].reg;
  assignValue(compiler, &ret, seqSlot);

  // Create another hidden local for the iterator object.
  null(compiler, false, &ret);
  int iterSlot = compiler->locals[addLocal(compiler, "iter ", 5)].reg;
  assignValue(compiler, &ret, iterSlot);
...
static void ifStatement(Compiler *compiler)
{
  ...
  if(ret.type == RET_CONST){
    ...
    bool cond = AS_BOOL(fodiTruth(compiler->parser->vm, ret.value));
...
{
  InstBuffer *code = &compiler->fn->code;
  int callIndex = code->count - 1;
  Instruction *call = &code->data[callIndex];

  setInstructionField(call, Field_OP, OP_TAILCALL);
}
...
//
// Construction is a two-stage process in Fodi that involves two separate
                         ...
                         bool isStatic, int methodSymbol)
{
  ...
  // Define the method.
  emitInstruction(compiler,
                  makeInstructionAsBx(OP_METHOD, AS_NUM(ret.value), methodSymbol, isStatic));
}
                         ...
                         const char *name, int length)
{
  ...
  fodiIntBufferWrite(compiler->parser->vm, methods, symbol);
  return symbol;
}
...
static bool method(Compiler *compiler, Variable classVariable)
{
  ...
  if (isForeign)
  {
    ...
    emitConstant(compiler, fodiNewStringLength(compiler->parser->vm, fullSignature, length), &ret);
    assignValue(compiler, &ret, reserveRegister(compiler));
...
static void classDefinition(Compiler *compiler, bool isForeign)
{
  ...
  Value classNameString = fodiNewStringLength(compiler->parser->vm,
  ...
  emitInstruction(compiler,
                  makeInstructionABx(OP_LOADK, reserveRegister(compiler), addConstant(compiler, ret.value)));

  // Load the superclass (if there is one).
  if (match(compiler, TOKEN_IS))
  {
    parsePrecedence(compiler, PREC_CALL, &ret);
  }
  else
  {
  ...
  numRegFieldsInstruction = emitInstruction(compiler,
                                            makeInstructionAsBx(OP_CLASS, ret.type == RET_REG ? AS_NUM(ret.value) : tempRegister(compiler), 0, isForeign));
  ...
  // bytecode will be adjusted by [fodiBindMethod] to take inherited fields
  // into account.
  fodiSymbolTableInit(&classInfo.fields);

  // Set up symbol buffers to track duplicate static and instance methods.
  fodiIntBufferInit(&classInfo.methods);
  fodiIntBufferInit(&classInfo.staticMethods);
  ...
  if (hasAttr)
  {
    ...
    emitInstruction(compiler,
                    makeInstructionABC(OP_ENDCLASS, classStart, 0, 0, 0));
  }

  // Update the class with the number of fields.
  if (!isForeign)
  {
    setInstructionField(&compiler->fn->code.data[numRegFieldsInstruction],
  ...
  fodiSymbolTableClear(compiler->parser->vm, &classInfo.fields);
  fodiIntBufferClear(compiler->parser->vm, &classInfo.methods);
  fodiIntBufferClear(compiler->parser->vm, &classInfo.staticMethods);
  compiler->enclosingClass = NULL;
  popScope(compiler);
  compiler->freeRegister = classStart;
}
...
static void import(Compiler *compiler)
{
  ...
  do
  {
    ...
    int sourceVariableConstant = addConstant(compiler,
                                             fodiNewStringLength(compiler->parser->vm,
...
static void variableDefinition(Compiler *compiler)
{
  ...
  defineVariable(compiler, symbol, &ret);
}
...
ObjFn *fodiCompile(FodiVM *vm, ObjModule *module, const char *source,
                   bool isExpression, bool printErrors)
{
  ...
  else
  {
    ...
    emitInstruction(&compiler,
                    makeInstructionABC(OP_RETURN, 0, 0, 1, 0));
  }
...
void fodiBindMethodCode(ObjClass *classObj, ObjClosure *close, Value *stack)
{
  int rip = 0;
  for (;;)
  {
    Instruction code = (Instruction)close->fn->code.data[rip];
    ...
    case OP_SETFIELD:
      ...
      setInstructionField(&close->fn->code.data[rip], Field_C, GET_C(code) + classObj->superclass->numFields);
      break;

    case OP_CALLSUPER:
    {
      Instruction prev = (Instruction)close->fn->code.data[rip - 1];
    ...
    case OP_CLOSURE:
    {
      ...
      fodiBindMethodCode(classObj, AS_CLOSURE(close->fn->constants.data[constant]), stack);
      break;
    }
    ...
    if (++rip >= close->fn->code.count)
      return;
  }
}

void fodiMarkCompiler(FodiVM *vm, Compiler *compiler)
{
  fodiGrayValue(vm, compiler->parser->current.value);
  fodiGrayValue(vm, compiler->parser->previous.value);
  fodiGrayValue(vm, compiler->parser->next.value);

  // Walk up the parent chain to mark the outer compilers too. The VM only
  // tracks the innermost one.
  do
  {
    fodiGrayObj(vm, (Obj *)compiler->fn);
    fodiGrayObj(vm, (Obj *)compiler->constants);
    fodiGrayObj(vm, (Obj *)compiler->attributes);

    if (compiler->enclosingClass != NULL)
    {
      fodiBlackenSymbolTable(vm, &compiler->enclosingClass->fields);

      if (compiler->enclosingClass->methodAttributes != NULL)
      {
        fodiGrayObj(vm, (Obj *)compiler->enclosingClass->methodAttributes);
      }
      if (compiler->enclosingClass->classAttributes != NULL)
      {
        fodiGrayObj(vm, (Obj *)compiler->enclosingClass->classAttributes);
      }
    }
...
static void disallowAttributes(Compiler *compiler)
{
  if (compiler->numAttributes > 0)
  {
    ...
    fodiMapClear(compiler->parser->vm, compiler->attributes);
    compiler->numAttributes = 0;
  }
}
...
{
  FodiVM *vm = compiler->parser->vm;

  if (IS_OBJ(group))
    fodiPushRoot(vm, AS_OBJ(group));
  if (IS_OBJ(key))
    fodiPushRoot(vm, AS_OBJ(key));
  if (IS_OBJ(value))
    fodiPushRoot(vm, AS_OBJ(value));

  Value groupMapValue = fodiMapGet(compiler->attributes, group);
  if (IS_UNDEFINED(groupMapValue))
  {
    groupMapValue = OBJ_VAL(fodiNewMap(vm));
    fodiMapSet(vm, compiler->attributes, group, groupMapValue);
  }
  ...
  Value keyItemsValue = fodiMapGet(groupMap, key);
  if (IS_UNDEFINED(keyItemsValue))
  {
    keyItemsValue = OBJ_VAL(fodiNewList(vm, 0));
    fodiMapSet(vm, groupMap, key, keyItemsValue);
  }

  // keyItems.add(value)
  ObjList *keyItems = AS_LIST(keyItemsValue);
  fodiValueBufferWrite(vm, &keyItems->elements, value);
...
static void emitAttributes(Compiler *compiler, ObjMap *attributes, ReturnValue* ret)
{
  ...
  for (uint32_t groupIdx = 0; groupIdx < attributes->capacity; groupIdx++)
  {
    ...
    if((groupIndex = addConstant(compiler, groupRet.value)) >= UINT8_MAX)
    {
      int keyReg = reserveRegister(compiler);
      emitInstruction(compiler,
                      makeInstructionABx(OP_LOADK, keyReg, groupIndex));
    ...
    for (uint32_t itemIdx = 0; itemIdx < groupItems->capacity; itemIdx++)
    {
      ...
      if((keyIndex = addConstant(compiler, keyRet.value)) >= UINT8_MAX)
      {
        int keyReg = reserveRegister(compiler);
        emitInstruction(compiler,
                        makeInstructionABx(OP_LOADK, keyReg, keyIndex));
      ...
      emitInstruction(compiler,
                      makeInstructionABC(OP_SETSUB, valueReg, mapReg, 
                        keyRet.type == RET_CONST ? keyIndex : AS_NUM(keyRet.value), keyRet.type == RET_CONST ? 1 : 0));
    }

    // Add the key/value to the map
    emitInstruction(compiler,
                makeInstructionABC(OP_SETSUB, mapReg, corestart, 
                  groupRet.type == RET_CONST ? groupIndex : AS_NUM(groupRet.value), groupRet.type == RET_CONST ? 1 : 0));
  }
...
static void emitAttributeMethods(Compiler *compiler, ObjMap *attributes)
{
  ...
  for (uint32_t methodIdx = 0; methodIdx < attributes->capacity; methodIdx++)
  {
    ...
    emitInstruction(compiler,
                    makeInstructionABC(OP_SETSUB, AS_NUM(ret.value), corestart, 
                      keyRet.type == RET_CONST ? keyIndex : AS_NUM(keyRet.value), keyRet.type == RET_CONST ? 1 : 0));
  }
  compiler->freeRegister = corestart;
}
...
static void copyAttributes(Compiler *compiler, ObjMap *into)
{
  ...
  for (uint32_t attrIdx = 0; attrIdx < compiler->attributes->capacity; attrIdx++)
  {
    ...
    if (IS_UNDEFINED(attrEntry->key))
      continue;
    fodiMapSet(vm, into, attrEntry->key, attrEntry->value);
  }

  fodiMapClear(vm, compiler->attributes);
}
                                 ...
                                 bool isStatic, const char *fullSignature, int32_t length)
{
  ...
  ObjMap *methodAttr = fodiNewMap(vm);
  fodiPushRoot(vm, (Obj *)methodAttr);
  ...
  if (compiler->enclosingClass->methodAttributes == NULL)
  {
    compiler->enclosingClass->methodAttributes = fodiNewMap(vm);
  }

  // Store the method attributes in the class map
  Value key = fodiNewStringLength(vm, fullSignatureWithPrefix, fullLength);
  fodiMapSet(vm, compiler->enclosingClass->methodAttributes, key, OBJ_VAL(methodAttr));
