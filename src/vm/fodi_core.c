...
DEF_PRIMITIVE(fiber_new)
{
  ...
  RETURN_OBJ(fodiNewFiber(vm, closure));
}
...
static bool runFiber(FodiVM *vm, ObjFiber *fiber, Value *args, bool isCall,
                             bool hasValue, const char *verb)
{
  ...
  if (fiber->numFrames == 1 &&
      fiber->frames[0].rip == fiber->frames[0].closure->fn->code.data)
  {
...
DEF_PRIMITIVE(fiber_isDone)
{
  ...
  RETURN_BOOL(runFiber->numFrames == 0 || fodiHasError(runFiber));
}
...
static void call_fn(FodiVM *vm, Value *args, int numArgs)
{
  fodiCallFunction(vm, vm->fiber, AS_CLOSURE(args[0]), args, numArgs + 1, -1);
}
...
DEF_PRIMITIVE(list_filled)
{
  ...
  ObjList *list = fodiNewList(vm, size);
...
{
  RETURN_OBJ(fodiNewList(vm, 0));
}

DEF_PRIMITIVE(list_add)
{
  fodiValueBufferWrite(vm, &AS_LIST(args[0])->elements, args[1]);
  RETURN_VAL(args[1]);
}
...
{
  fodiValueBufferClear(vm, &AS_LIST(args[0])->elements);
  RETURN_NULL;
}
...
DEF_PRIMITIVE(list_insert)
{
  ...
  fodiListInsert(vm, list, args[2], index);
  RETURN_VAL(args[2]);
}
...
DEF_PRIMITIVE(list_removeAt)
{
  ...
  RETURN_VAL(fodiListRemoveAt(vm, list, index));
}

DEF_PRIMITIVE(list_removeValue)
{
  ObjList *list = AS_LIST(args[0]);
  int index = fodiListIndexOf(vm, list, args[1]);
  if (index == -1)
    RETURN_NULL;
  RETURN_VAL(fodiListRemoveAt(vm, list, index));
}
...
{
  ObjList *list = AS_LIST(args[0]);
  RETURN_NUM(fodiListIndexOf(vm, list, args[1]));
}
...
DEF_PRIMITIVE(map_subscript)
{
  ...
  Value value = fodiMapGet(map, args[1]);
...
DEF_PRIMITIVE(map_subscriptSetter)
{
  ...
  fodiMapSet(vm, AS_MAP(args[0]), args[1], args[2]);
  RETURN_VAL(args[2]);
}
...
DEF_PRIMITIVE(map_containsKey)
{
  ...
  RETURN_BOOL(!IS_UNDEFINED(fodiMapGet(AS_MAP(args[0]), args[1])));
}
...
DEF_PRIMITIVE(map_remove)
{
  ...
  RETURN_VAL(fodiMapRemoveKey(vm, AS_MAP(args[0]), args[1]));
}
...
DEF_NUM_CONSTANT(nan, FODI_DOUBLE_NAN)
...
DEF_PRIMITIVE(num_dotDot)
{
  ...
  RETURN_VAL(fodiNewRange(vm, from, to, true));
}
...
DEF_PRIMITIVE(num_dotDotDot)
{
  ...
  double from = AS_NUM(args[0]);
  double to = AS_NUM(args[1]);
  RETURN_VAL(fodiNewRange(vm, from, to, false));
}
...
{
  RETURN_VAL(fodiNumToString(vm, AS_NUM(args[0])));
}
...
{
  RETURN_BOOL(fodiValuesEqual(args[1], args[2]));
}
...
{
  RETURN_BOOL(fodiValuesEqual(args[0], args[1]));
}
...
DEF_PRIMITIVE(object_is)
{
  ...
  ObjClass *classObj = fodiGetClass(vm, args[0]);
...
DEF_PRIMITIVE(object_toString)
{
  ...
  RETURN_VAL(fodiStringFormat(vm, "instance of @", name));
}

DEF_PRIMITIVE(object_type)
{
  RETURN_OBJ(fodiGetClass(vm, args[0]));
}
...
DEF_PRIMITIVE(range_toString)
{
  ...
  Value from = fodiNumToString(vm, range->from);
  fodiPushRoot(vm, AS_OBJ(from));

  Value to = fodiNumToString(vm, range->to);
  fodiPushRoot(vm, AS_OBJ(to));

  Value result = fodiStringFormat(vm, "@$@", from,
...
DEF_PRIMITIVE(string_fromCodePoint)
{
  ...
  RETURN_VAL(fodiStringFromCodePoint(vm, codePoint));
}
...
DEF_PRIMITIVE(string_fromByte)
{
  ...
  RETURN_VAL(fodiStringFromByte(vm, (uint8_t)byte));
}
...
DEF_PRIMITIVE(string_codePointAt)
{
  ...
  RETURN_NUM(fodiUtf8Decode((uint8_t *)string->value + index,
                            string->length - index));
}
...
DEF_PRIMITIVE(string_contains)
{
  ...
  RETURN_BOOL(fodiStringFind(string, search, 0) != UINT32_MAX);
}
...
DEF_PRIMITIVE(string_indexOf1)
{
  ...
  uint32_t index = fodiStringFind(string, search, 0);
...
DEF_PRIMITIVE(string_indexOf2)
{
  ...
  uint32_t index = fodiStringFind(string, search, start);
  RETURN_NUM(index == UINT32_MAX ? -1 : (int)index);
}
...
DEF_PRIMITIVE(string_iteratorValue)
{
  ...
  RETURN_VAL(fodiStringCodePointAt(vm, string, index));
}
...
DEF_PRIMITIVE(string_plus)
{
  if (!validateString(vm, args[1], "Right operand"))
    return false;
  RETURN_VAL(fodiStringFormat(vm, "@@", args[0], args[1]));
}
...
DEF_PRIMITIVE(string_subscript)
{
  ...
  RETURN_VAL(fodiNewStringFromRange(vm, string, start, count, step));
}
...
static ObjClass *defineClass(FodiVM *vm, ObjModule *module, const char *name)
{
  ObjString *nameString = AS_STRING(fodiNewString(vm, name));
  fodiPushRoot(vm, (Obj *)nameString);

  ObjClass *classObj = fodiNewSingleClass(vm, 0, nameString);

  fodiDefineVariable(vm, module, name, nameString->length, OBJ_VAL(classObj), NULL);

  fodiPopRoot(vm);
  return classObj;
}

void fodiInitializeCore(FodiVM *vm)
{
  ObjModule *coreModule = fodiNewModule(vm, NULL);
  fodiPushRoot(vm, (Obj *)coreModule);

  // The core module's key is null in the module map.
  fodiMapSet(vm, vm->modules, NULL_VAL, OBJ_VAL(coreModule));
  fodiPopRoot(vm); // coreModule.
  ...
  fodiBindSuperclass(vm, vm->classClass, vm->objectClass);
  ...
  // collected.
  fodiBindSuperclass(vm, objectMetaclass, vm->classClass);
  ...
  fodiInterpret(vm, NULL, coreModuleSource);

  vm->boolClass = AS_CLASS(fodiFindVariable(vm, coreModule, "Bool"));
  PRIMITIVE(vm->boolClass, "toString", bool_toString);
  PRIMITIVE(vm->boolClass, "!", bool_not);

  vm->fiberClass = AS_CLASS(fodiFindVariable(vm, coreModule, "Fiber"));
  ...
  vm->fnClass = AS_CLASS(fodiFindVariable(vm, coreModule, "Fn"));
  ...
  vm->nullClass = AS_CLASS(fodiFindVariable(vm, coreModule, "Null"));
  PRIMITIVE(vm->nullClass, "!", null_not);
  PRIMITIVE(vm->nullClass, "toString", null_toString);

  vm->numClass = AS_CLASS(fodiFindVariable(vm, coreModule, "Num"));
  ...
  vm->stringClass = AS_CLASS(fodiFindVariable(vm, coreModule, "String"));
  ...
  vm->listClass = AS_CLASS(fodiFindVariable(vm, coreModule, "List"));
  ...
  vm->mapClass = AS_CLASS(fodiFindVariable(vm, coreModule, "Map"));
  vm->mapEntryClass = AS_CLASS(fodiFindVariable(vm, coreModule, "MapEntry"));
  ...
  vm->rangeClass = AS_CLASS(fodiFindVariable(vm, coreModule, "Range"));
  ...
  ObjClass *systemClass = AS_CLASS(fodiFindVariable(vm, coreModule, "System"));
