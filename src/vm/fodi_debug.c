...
void fodiDebugPrintStackTrace(FodiVM *vm)
{
  ...
  {
    vm->config.errorFn(vm, FODI_ERROR_RUNTIME,
  ...
  for (int i = fiber->numFrames - 1; i >= 0; i--)
  {
    ...
    // detail of what part of the core module is written in C and what is Fodi.
    if (fn->module->name == NULL)
      continue;

    // -1 because IP has advanced past the instruction that it just executed.
    int line = fn->debug->sourceLines.data[frame->rip - fn->code.data - 1];
    vm->config.errorFn(vm, FODI_ERROR_STACK_TRACE,
...
void fodiDumpConstants(ObjFn* func)
{
  ...
  for (int i = 0; i < func->constants.count; i++){
    printf("[%d] ", i);
    fodiDumpValue(func->constants.data[i]);
    printf(" | ");
...
static int dumpInstruction(FodiVM *vm, ObjFn *fn, int i, int *lastLine)
{
  int start = i;

  if (start >= fn->code.count)
  {
    return -1;
  }

  Instruction *bytecode = fn->code.data;
  ...
  case OP_LOADK:
    ...
    if (fn->constants.count <= GET_Bx(code))
      ...
      fodiDumpValue(fn->constants.data[GET_Bx(code)]);
    printf(" ]");
    break;
  ...
  case OP_CALL:
    printABC("CALL", GET_A(code), GET_vB(code), GET_vC(code), GET_K(code));
    printABCGap();
  ...
  case OP_CALLSUPER:
    printABC("CALLSUPER", GET_A(code), GET_vB(code), GET_vC(code), GET_K(code));
    printABCGap();
    printf("'%s'", vm->methodNames.data[GET_vC(code)]->value);
    break;
