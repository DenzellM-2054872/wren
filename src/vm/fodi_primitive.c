...
static uint32_t validateIndexValue(FodiVM *vm, uint32_t count, double value,
                                   const char *argName)
{
  ...
  vm->fiber->error = fodiStringFormat(vm, "$ out of bounds.", argName);
  return UINT32_MAX;
}
