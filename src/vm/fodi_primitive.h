#ifndef fodi_primitive_h
#define fodi_primitive_h

#include "fodi_vm.h"

// Binds a primitive method named [name] (in Fodi) implemented using C function
// [fn] to `ObjClass` [cls].
#define PRIMITIVE(cls, name, function)                                        \
  do                                                                          \
  {                                                                           \
    int symbol = fodiSymbolTableEnsure(vm,                                    \
                                       &vm->methodNames, name, strlen(name)); \
    Method method;                                                            \
    method.type = METHOD_PRIMITIVE;                                           \
    method.as.primitive = prim_##function;                                    \
    fodiBindMethod(vm, cls, symbol, method);                                  \
  } while (false)
...
#define DEF_PRIMITIVE(name) \
  static bool prim_##name(FodiVM *vm, Value *args)
...
#define RETURN_ERROR(msg)                                             \
  do                                                                  \
  {                                                                   \
    vm->fiber->error = fodiNewStringLength(vm, msg, sizeof(msg) - 1); \
    return false;                                                     \
  } while (false)
...
// `UINT32_MAX`.
uint32_t validateIndex(FodiVM *vm, Value arg, uint32_t count,
                       const char *argName);

// Validates that the given [arg] is a String. Returns true if it is. If not,
// reports an error and returns false.
bool validateString(FodiVM *vm, Value arg, const char *argName);
...
// range is decreasing.
uint32_t calculateRange(FodiVM *vm, ObjRange *range, uint32_t *length,
                        int *step);
