...
{
  fodiStringBufferInit(symbols);
}

void fodiSymbolTableClear(FodiVM *vm, SymbolTable *symbols)
{
  fodiStringBufferClear(vm, symbols);
}

int fodiSymbolTableAdd(FodiVM *vm, SymbolTable *symbols,
                       const char *name, size_t length)
{
  ObjString *symbol = AS_STRING(fodiNewStringLength(vm, name, length));

  fodiPushRoot(vm, &symbol->obj);
  fodiStringBufferWrite(vm, symbols, symbol);
  fodiPopRoot(vm);
                          ...
                          const char *name, size_t length)
{
  ...
  int existing = fodiSymbolTableFind(symbols, name, length);
  if (existing != -1)
    return existing;

  // New symbol, so add it.
  return fodiSymbolTableAdd(vm, symbols, name, length);
}
                        ...
                        const char *name, size_t length)
{
  ...
  for (int i = 0; i < symbols->count; i++)
  {
    ...
    if (fodiStringEqualsCString(symbols->data[i], name, length))
      return i;
  }
...
void fodiBlackenSymbolTable(FodiVM *vm, SymbolTable *symbolTable)
{
  ...
  {
    fodiGrayObj(vm, &symbolTable->data[i]->obj);
  }
