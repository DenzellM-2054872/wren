...
static void initObj(FodiVM *vm, Obj *obj, ObjType type, ObjClass *classObj)
{
  obj->type = type;
  obj->isDark = false;
...
ObjClass *fodiNewSingleClass(FodiVM *vm, int numFields, ObjString *name)
{
  ...
  fodiPushRoot(vm, (Obj *)classObj);
  fodiMethodBufferInit(&classObj->methods);
  fodiPopRoot(vm);

  return classObj;
}

void fodiBindSuperclass(FodiVM *vm, ObjClass *subclass, ObjClass *superclass)
{
  ...
  {
    fodiBindMethod(vm, subclass, i, superclass->methods.data[i]);
  }
}

ObjClass *fodiNewClass(FodiVM *vm, ObjClass *superclass, int numFields,
                       ObjString *name)
{
  // Create the metaclass.
  Value metaclassName = fodiStringFormat(vm, "@ metaclass", OBJ_VAL(name));
  fodiPushRoot(vm, AS_OBJ(metaclassName));

  ObjClass *metaclass = fodiNewSingleClass(vm, 0, AS_STRING(metaclassName));
  metaclass->obj.classObj = vm->classClass;

  fodiPopRoot(vm);

  // Make sure the metaclass isn't collected when we allocate the class.
  fodiPushRoot(vm, (Obj *)metaclass);

  // Metaclasses always inherit Class and do not parallel the non-metaclass
  // hierarchy.
  fodiBindSuperclass(vm, metaclass, vm->classClass);

  ObjClass *classObj = fodiNewSingleClass(vm, numFields, name);
  ...
  fodiBindSuperclass(vm, classObj, superclass);
...
void fodiBindMethod(FodiVM *vm, ObjClass *classObj, int symbol, Method method)
{
  ...
  if (symbol >= classObj->methods.count)
  {
    ...
    fodiMethodBufferFill(vm, &classObj->methods, noMethod,
...
CompilerUpvalue *fodiNewProtoUpvalue(FodiVM *vm, bool local, int index)
{
...
ObjClosure *fodiNewClosure(FodiVM *vm, ObjFn *fn, bool isProto)
{
...
ObjFiber *fodiNewFiber(FodiVM *vm, ObjClosure *closure)
{
  ...
  int stackCapacity = closure == NULL
                          ? 1
                          : fodiPowerOf2Ceil(closure->fn->maxSlots + 1);
  ...
  if (closure != NULL)
  {
    ...
    fodiAppendCallFrame(vm, fiber, closure, fiber->stack, -1);
...
void fodiEnsureStack(FodiVM *vm, ObjFiber *fiber, int needed)
{
  if (fiber->stackCapacity >= needed)
    return;

  int capacity = fodiPowerOf2Ceil(needed);

  Value *oldStack = fiber->stack;
  int oldCapacity = fiber->stackCapacity;
  fiber->stack = (Value *)fodiReallocate(vm, fiber->stack,
...
static char* getType(FodiVM *vm, Value value)
{
...
ObjForeign *fodiNewForeign(FodiVM *vm, ObjClass *classObj, size_t size)
{
...
ObjFn *fodiNewFunction(FodiVM *vm, ObjModule *module, int maxSlots)
{
  FnDebug *debug = ALLOCATE(vm, FnDebug);
  debug->name = NULL;
  fodiIntBufferInit(&debug->sourceLines);

  ObjFn *fn = ALLOCATE(vm, ObjFn);
  initObj(vm, &fn->obj, OBJ_FN, vm->fnClass);

  fodiValueBufferInit(&fn->constants);
  fodiInstBufferInit(&fn->code);
  fodiIntBufferInit(&fn->stackTop);
...
void fodiFunctionBindName(FodiVM *vm, ObjFn *fn, const char *name, int length)
{
...
Value fodiNewInstance(FodiVM *vm, ObjClass *classObj)
{
...
ObjList *fodiToList(FodiVM *vm, Value value)
{
  if (IS_LIST(value))
  {
  ...
  if (IS_RANGE(value))
  {
    ...
    ObjList *list = fodiNewList(vm, length + 1);
  ...
  if (IS_STRING(value))
  {
    ...
    ObjList *list = fodiNewList(vm, str->length);
    for (size_t i = 0; i < str->length; i++)
    {
      list->elements.data[i] = fodiNewStringLength(vm, &str->value[i], 1);
    }
    return list;
  }

  vm->fiber->error = fodiStringFormat(vm, "$$", getType(vm, value), " does not implement 'iterate(_)'.");
  return NULL;
}

ObjList *fodiConcatList(FodiVM *vm, ObjList *list1, ObjList *list2)
{
  size_t count1 = list1->elements.count;
  if(!list2) return NULL;
  size_t count2 = list2->elements.count;
  ObjList *newList = fodiNewList(vm, count1 + count2);
...
ObjList *fodiNewList(FodiVM *vm, uint32_t numElements)
{
...
ObjList *fodiRepeatList(FodiVM *vm, ObjList *list, size_t times)
{
  size_t originalCount = list->elements.count;
  size_t newCount = originalCount * times;
  ObjList *newList = fodiNewList(vm, newCount);
...
void fodiListInsert(FodiVM *vm, ObjList *list, Value value, uint32_t index)
{
  if (IS_OBJ(value))
    fodiPushRoot(vm, AS_OBJ(value));

  // Add a slot at the end of the list.
  fodiValueBufferWrite(vm, &list->elements, NULL_VAL);
...
int fodiListIndexOf(FodiVM *vm, ObjList *list, Value value)
{
  int count = list->elements.count;
  for (int i = 0; i < count; i++)
  {
    Value item = list->elements.data[i];
    if (fodiValuesEqual(item, value))
    {
      return i;
    }
  }
...
Value fodiListRemoveAt(FodiVM *vm, ObjList *list, uint32_t index)
{
  Value removed = list->elements.data[index];

  if (IS_OBJ(removed))
    fodiPushRoot(vm, AS_OBJ(removed));
  ...
  {
    list->elements.data = (Value *)fodiReallocate(vm, list->elements.data,
...
ObjMapEntry *fodiNewMapEntry(FodiVM *vm, MapEntry *entry)
{
...
ObjMap *fodiCopyMap(FodiVM *vm, ObjMap *original)
{
  ObjMap *map = fodiNewMap(vm);
...
static inline uint32_t hashNumber(double num)
{
  ...
  return hashBits(fodiDoubleToBits(num));
}
                      ...
                      MapEntry **result)
{
  ...
  do
  {
    ...
    else if (fodiValuesEqual(entry->key, key))
    {
      // We found the key.
      *result = entry;
      return true;
...
static void resizeMap(FodiVM *vm, ObjMap *map, uint32_t capacity)
{
...
Value fodiMapGet(ObjMap *map, Value key)
{
  MapEntry *entry;
...
void fodiMapSet(FodiVM *vm, ObjMap *map, Value key, Value value)
{
...
void fodiMapClear(FodiVM *vm, ObjMap *map)
{
  DEALLOCATE(vm, map->entries);
  map->entries = NULL;
  map->capacity = 0;
...
Value fodiMapRemoveKey(FodiVM *vm, ObjMap *map, Value key)
{
  MapEntry *entry;
  if (!findEntry(map->entries, map->capacity, key, &entry))
    return NULL_VAL;
...
ObjModule *fodiNewModule(FodiVM *vm, ObjString *name)
{
  ...
  fodiSymbolTableInit(&module->variableNames);
  fodiValueBufferInit(&module->variables);
...
Value fodiNewRange(FodiVM *vm, double from, double to, bool isInclusive)
{
...
// hash.
static ObjString *allocateString(FodiVM *vm, size_t length)
{
...
Value fodiNewString(FodiVM *vm, const char *text)
{
  return fodiNewStringLength(vm, text, strlen(text));
}

Value fodiNewStringLength(FodiVM *vm, const char *text, size_t length)
{
...
Value fodiRepeatString(FodiVM *vm, const char *text, size_t repeats)
{
  // Allow NULL if the string is empty since byte buffers don't allocate any
  // characters for a zero-length string.
...
Value fodiConcatString(FodiVM *vm, const char *text1, const char *text2)
{
...
Value fodiNewStringFromRange(FodiVM *vm, ObjString *source, int start,
                             uint32_t count, int step)
{
  ...
  {
    length += fodiUtf8DecodeNumBytes(from[start + i * step]);
  }
  ...
  for (uint32_t i = 0; i < count; i++)
  {
    ...
    int codePoint = fodiUtf8Decode(from + index, source->length - index);

    if (codePoint != -1)
    {
      to += fodiUtf8Encode(codePoint, to);
    }
  }
...
Value fodiNumToString(FodiVM *vm, double value)
{
  ...
  return fodiNewStringLength(vm, buffer, length);
}

Value fodiStringFromCodePoint(FodiVM *vm, int value)
{
  int length = fodiUtf8EncodeNumBytes(value);
  ASSERT(length != 0, "Value out of range.");

  ObjString *string = allocateString(vm, length);

  fodiUtf8Encode(value, (uint8_t *)string->value);
  hashString(string);

  return OBJ_VAL(string);
}

Value fodiStringFromByte(FodiVM *vm, uint8_t value)
{
  int length = 1;
  ObjString *string = allocateString(vm, length);
...
Value fodiStringFormat(FodiVM *vm, const char *format, ...)
{
  va_list argList;
...
Value fodiStringCodePointAt(FodiVM *vm, ObjString *string, uint32_t index)
{
  ASSERT(index < string->length, "Index out of bounds.");

  int codePoint = fodiUtf8Decode((uint8_t *)string->value + index,
                                 string->length - index);
  if (codePoint == -1)
  {
    ...
    bytes[1] = '\0';
    return fodiNewStringLength(vm, bytes, 1);
  }

  return fodiStringFromCodePoint(vm, codePoint);
}

// Uses the Boyer-Moore-Horspool string matching algorithm.
uint32_t fodiStringFind(ObjString *haystack, ObjString *needle, uint32_t start)
{
...
static Value fodiSetSubscriptList(FodiVM *vm, ObjList *list, Value subscript, Value value)
{
...
static Value fodiSetSubscriptMap(FodiVM *vm, ObjMap *map, Value subscript, Value value)
{
  if (!validateKey(vm, subscript))
    return false;

  fodiMapSet(vm, map, subscript, value);
  return value;
}

Value fodiSetSubscript(FodiVM *vm, Value sequence, Value subscript, Value value)
{
  if (IS_LIST(sequence)){
    return fodiSetSubscriptList(vm, AS_LIST(sequence), subscript, value);
  }
  if (IS_MAP(sequence)){
    return fodiSetSubscriptMap(vm, AS_MAP(sequence), subscript, value);
  }

  vm->fiber->error = fodiStringFormat(vm, "$$", getType(vm, sequence), " does not implement '[_]=(_)'.");
  return false;
}

static Value fodiSubscriptList(FodiVM *vm, ObjList *list, Value subscript)
{
  if (IS_NUM(subscript))
  {
    uint32_t index = validateIndex(vm, subscript, list->elements.count,
  ...
  {
    vm->fiber->error = fodiNewString(vm, "Subscript must be a number or a range.");
    return false;
  }
  ...
  ObjList *result = fodiNewList(vm, count);
  for (uint32_t i = 0; i < count; i++)
  {
...
static Value fodiSubscriptMap(FodiVM *vm, ObjMap *map, Value subscript)
{
  if (!validateKey(vm, subscript))
    return false;

  Value value = fodiMapGet(map, subscript);
...
static Value fodiSubscriptString(FodiVM *vm, ObjString *string, Value subscript)
{
  if (IS_NUM(subscript))
  {
    ...
    return fodiStringCodePointAt(vm, string, index);
  }
  ...
  return fodiNewStringFromRange(vm, string, start, count, step);
}

Value fodiSubscript(FodiVM *vm, Value sequence, Value subscript)
{
  if (IS_LIST(sequence)){
    return fodiSubscriptList(vm, AS_LIST(sequence), subscript);
  }
  if (IS_MAP(sequence)){
    return fodiSubscriptMap(vm, AS_MAP(sequence), subscript);
  }
  if (IS_STRING(sequence)){
    return fodiSubscriptString(vm, AS_STRING(sequence), subscript);
  }

  vm->fiber->error = fodiStringFormat(vm, "$$", getType(vm, sequence), " does not implement '[_]'.");
  return false;
}

static Value mapIteratorValue(FodiVM *vm, ObjMap *map, Value iterator){
  ...
  return OBJ_VAL(fodiNewMapEntry(vm, entry));
}

static Value listIteratorValue(FodiVM *vm, ObjList *list, Value iterator){
...
static Value stringIteratorValue(FodiVM *vm, ObjString *string, Value iterator){
...
Value fodiIteratorValue(FodiVM *vm, Value sequence, Value iterator){
  if(IS_LIST(sequence)){
  ...
  vm->fiber->error = fodiStringFormat(vm, "$$", getType(vm, sequence), " does not implement 'iterateorValue(_)'.");
  return NULL_VAL;
}

static Value fodiIterateList(FodiVM *vm, ObjList *list, Value iterator) 
{
...
static Value fodiIterateMap(FodiVM *vm, ObjMap *map, Value iterator) 
{
  if (map->count == 0)
    return FALSE_VAL;
...
static Value fodiIterateRange(FodiVM *vm, ObjRange *range, Value iterator) 
{
...
static Value fodiIterateString(FodiVM *vm, ObjString *string, Value iterator) 
{
  // If we're starting the iteration, return the first index.
  if (IS_NULL(iterator))
  {
...
Value fodiIterate(FodiVM *vm, Value sequence, Value iterator){
  if(IS_LIST(sequence)){
    return fodiIterateList(vm, AS_LIST(sequence), iterator);
  }
  if(IS_MAP(sequence)){
    return fodiIterateMap(vm, AS_MAP(sequence), iterator);
  }
  if(IS_RANGE(sequence)){
    return fodiIterateRange(vm, AS_RANGE(sequence), iterator);
  }
  if(IS_STRING(sequence)){
    return fodiIterateString(vm, AS_STRING(sequence), iterator);
  }

  vm->fiber->error = fodiStringFormat(vm, "$$", getType(vm, sequence), " does not implement 'iterate(_)'.");
  return NULL_VAL;

}

Value fodiNegative(FodiVM *vm, Value value)
{
  if (IS_NUM(value))
  {
...
Value fodiTruth(FodiVM *vm, Value value)
{
  if (IS_BOOL(value))
  {
    return value;
...
Value fodiNot(FodiVM *vm, Value value)
{
  if (IS_BOOL(value))
  {
...
Value fodiAddList(FodiVM *vm, ObjList *list, Value value, bool isConcat)
{
  if (!isConcat)
  {
    fodiValueBufferWrite(vm, &list->elements, value);
    return value;
  }

  ObjList *valueList = fodiToList(vm, value);
  if(!valueList) return NULL_VAL;
  ObjList *newList = fodiConcatList(vm, list, valueList);
  return OBJ_VAL(newList);
}

Value fodiAdd(FodiVM *vm, Value a, Value b)
{
  if (IS_NUM(a))
  {
    if (!IS_NUM(b))
  ...
  if (IS_STRING(a))
  {
    ...
    return OBJ_VAL(fodiStringFormat(vm, "@@", a, b));
  }

  if (IS_LIST(a))
  {
    return OBJ_VAL(fodiAddList(vm, AS_LIST(a), b, true));
  }

  vm->fiber->error = fodiStringFormat(vm, "$$", getType(vm, a), " does not implement '+(_)'.");
  return NULL_VAL;
}

Value fodiSubtract(FodiVM *vm, Value a, Value b)
{
  if (IS_NUM(a))
  {
    if (!IS_NUM(b))
...
Value fodiMultiply(FodiVM *vm, Value a, Value b)
{
  if (IS_NUM(a))
  {
    if (!IS_NUM(b))
  ...
  if (IS_STRING(a))
  {
    ...
    return OBJ_VAL(fodiRepeatString(vm, AS_CSTRING(a), (int)AS_NUM(b)));
  }
  ...
  if (IS_LIST(a))
  {
    ...
    // List repetition
    return OBJ_VAL(fodiRepeatList(vm, AS_LIST(a), (int)AS_NUM(b)));
  }
  vm->fiber->error = fodiStringFormat(vm, "$$", getType(vm, a), " does not implement '*(_)'.");
  return NULL_VAL;
}

Value fodiDivide(FodiVM *vm, Value a, Value b)
{
  if (!IS_NUM(a))
  {
    vm->fiber->error = CONST_STRING(vm, "Left operand must be a number.");
...
ObjUpvalue *fodiNewUpvalue(FodiVM *vm, Value *value)
{
...
void fodiGrayObj(FodiVM *vm, Obj *obj)
{
  if (obj == NULL)
    return;
...
void fodiGrayValue(FodiVM *vm, Value value)
{
  if (!IS_OBJ(value))
    return;
  fodiGrayObj(vm, AS_OBJ(value));
}

void fodiGrayBuffer(FodiVM *vm, ValueBuffer *buffer)
{
  for (int i = 0; i < buffer->count; i++)
  {
    fodiGrayValue(vm, buffer->data[i]);
  }
}

static void blackenClass(FodiVM *vm, ObjClass *classObj)
{
  // The metaclass.
  fodiGrayObj(vm, (Obj *)classObj->obj.classObj);

  // The superclass.
  fodiGrayObj(vm, (Obj *)classObj->superclass);
  ...
  for (int i = 0; i < classObj->methods.count; i++)
  {
    ...
    {
      fodiGrayObj(vm, (Obj *)classObj->methods.data[i].as.closure);
    }
  }

  fodiGrayObj(vm, (Obj *)classObj->name);

  if (!IS_NULL(classObj->attributes))
    fodiGrayObj(vm, AS_OBJ(classObj->attributes));
...
static void blackenClosure(FodiVM *vm, ObjClosure *closure)
{
  // Mark the function.
  fodiGrayObj(vm, (Obj *)closure->fn);

  // Mark the upvalues.
  for (int i = 0; i < closure->fn->numUpvalues; i++)
  {
    fodiGrayObj(vm, (Obj *)closure->upvalues[i]);
  }
...
static void blackenFiber(FodiVM *vm, ObjFiber *fiber)
{
  // Stack functions.
  for (int i = 0; i < fiber->numFrames; i++)
  {
    fodiGrayObj(vm, (Obj *)fiber->frames[i].closure);
  }

  // Stack variables.
  for (Value *slot = fiber->stack; slot < fiber->stackTop; slot++)
  {
    fodiGrayValue(vm, *slot);
  }
  ...
  while (upvalue != NULL)
  {
    fodiGrayObj(vm, (Obj *)upvalue);
    upvalue = upvalue->next;
  }

  // The caller.
  fodiGrayObj(vm, (Obj *)fiber->caller);
  fodiGrayValue(vm, fiber->error);
...
static void blackenFn(FodiVM *vm, ObjFn *fn)
{
  // Mark the constants.
  fodiGrayBuffer(vm, &fn->constants);

  // Mark the module it belongs to, in case it's been unloaded.
  fodiGrayObj(vm, (Obj *)fn->module);

  // Keep track of how much memory is still in use.
  vm->bytesAllocated += sizeof(ObjFn);
  vm->bytesAllocated += sizeof(Instruction) * fn->code.capacity;
...
static void blackenForeign(FodiVM *vm, ObjForeign *foreign)
{
...
static void blackenInstance(FodiVM *vm, ObjInstance *instance)
{
  fodiGrayObj(vm, (Obj *)instance->obj.classObj);

  // Mark the fields.
  for (int i = 0; i < instance->obj.classObj->numFields; i++)
  {
    fodiGrayValue(vm, instance->fields[i]);
  }
...
static void blackenList(FodiVM *vm, ObjList *list)
{
  // Mark the elements.
  fodiGrayBuffer(vm, &list->elements);
...
static void blackenMap(FodiVM *vm, ObjMap *map)
{
  // Mark the entries.
  for (uint32_t i = 0; i < map->capacity; i++)
  {
    ...
    fodiGrayValue(vm, entry->key);
    fodiGrayValue(vm, entry->value);
  }
...
static void blackenModule(FodiVM *vm, ObjModule *module)
{
  // Top-level variables.
  for (int i = 0; i < module->variables.count; i++)
  {
    fodiGrayValue(vm, module->variables.data[i]);
  }

  fodiBlackenSymbolTable(vm, &module->variableNames);

  fodiGrayObj(vm, (Obj *)module->name);

  // Keep track of how much memory is still in use.
  vm->bytesAllocated += sizeof(ObjModule);
}

static void blackenRange(FodiVM *vm, ObjRange *range)
{
  // Keep track of how much memory is still in use.
  vm->bytesAllocated += sizeof(ObjRange);
}

static void blackenString(FodiVM *vm, ObjString *string)
{
  // Keep track of how much memory is still in use.
  vm->bytesAllocated += sizeof(ObjString) + string->length + 1;
}

static void blackenUpvalue(FodiVM *vm, ObjUpvalue *upvalue)
{
  // Mark the closed-over object (in case it is closed).
  fodiGrayValue(vm, upvalue->closed);

  // Keep track of how much memory is still in use.
  vm->bytesAllocated += sizeof(ObjUpvalue);
}

static void blackenObject(FodiVM *vm, Obj *obj)
{
#if FODI_DEBUG_TRACE_MEMORY
  printf("mark ");
  fodiDumpValue(OBJ_VAL(obj));
  printf(" @ %p\n", obj);
#endif
...
void fodiBlackenObjects(FodiVM *vm)
{
...
void fodiFreeObj(FodiVM *vm, Obj *obj)
{
#if FODI_DEBUG_TRACE_MEMORY
  printf("free ");
  fodiDumpValue(OBJ_VAL(obj));
  ...
  {
  case OBJ_CLASS:
    fodiMethodBufferClear(vm, &((ObjClass *)obj)->methods);
    break;
  ...
  {
    ObjFn *fn = (ObjFn *)obj;
    fodiValueBufferClear(vm, &fn->constants);
    fodiInstBufferClear(vm, &fn->code);
    fodiIntBufferClear(vm, &fn->debug->sourceLines);
    fodiIntBufferClear(vm, &fn->stackTop);
  ...
  case OBJ_FOREIGN:
    fodiFinalizeForeign(vm, (ObjForeign *)obj);
    break;

  case OBJ_LIST:
    fodiValueBufferClear(vm, &((ObjList *)obj)->elements);
    break;
  ...
  case OBJ_MODULE:
    fodiSymbolTableClear(vm, &((ObjModule *)obj)->variableNames);
    fodiValueBufferClear(vm, &((ObjModule *)obj)->variables);
    break;
...
ObjClass *fodiGetClass(FodiVM *vm, Value value)
{
  return fodiGetClassInline(vm, value);
}

bool fodiValuesEqual(Value a, Value b)
{
  if (fodiValuesSame(a, b))
    return true;
  ...
  case OBJ_STRING:
  {
    ...
    return aString->hash == bString->hash &&
           fodiStringEqualsCString(aString, bString->value, bString->length);
  }
...
Value fodiValuesLess(FodiVM *vm, Value a, Value b, bool invert){
  if (!IS_NUM(a))
  {
    vm->fiber->error = CONST_STRING(vm, "Left operand must be a number.");
    return NULL_VAL;
...
Value fodiValuesLessEq(FodiVM *vm, Value a, Value b, bool invert){
  if (!IS_NUM(a))
  {
    vm->fiber->error = CONST_STRING(vm, "Left operand must be a number.");
    return NULL_VAL;
