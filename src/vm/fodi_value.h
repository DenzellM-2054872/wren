...
// Since Fodi is dynamically typed, any variable can hold a value of any type,
...
// storage location that can hold any Fodi value. The stack, module variables,
...
// Fodi implementation calls these "Obj", or objects, though to a user, all
// values are objects.
//
...
//
// The representation is controlled by the `FODI_NAN_TAGGING` define. If that's
...
#define AS_NUM(value) (fodiValueToNum(value))             // double
...
// These macros promote a primitive C value to a full Fodi Value. There are
...
#define NUM_VAL(num) (fodiNumToValue(num))                   // double
#define OBJ_VAL(obj) (fodiObjectToValue((Obj *)(obj)))       // Any Obj___*

// These perform type tests on a Value, returning `true` if the Value is of the
// given type.
#define IS_BOOL(value) (fodiIsBool(value))                      // Bool
#define IS_CLASS(value) (fodiIsObjType(value, OBJ_CLASS))       // ObjClass
#define IS_CLOSURE(value) (fodiIsObjType(value, OBJ_CLOSURE))   // ObjClosure
#define IS_FIBER(value) (fodiIsObjType(value, OBJ_FIBER))       // ObjFiber
#define IS_FN(value) (fodiIsObjType(value, OBJ_FN))             // ObjFn
#define IS_FOREIGN(value) (fodiIsObjType(value, OBJ_FOREIGN))   // ObjForeign
#define IS_INSTANCE(value) (fodiIsObjType(value, OBJ_INSTANCE)) // ObjInstance
#define IS_LIST(value) (fodiIsObjType(value, OBJ_LIST))         // ObjList
#define IS_MAPENTRY(value) (fodiIsObjType(value, OBJ_MAPENTRY)) // ObjMapEntry
#define IS_MAP(value) (fodiIsObjType(value, OBJ_MAP))           // ObjMap
#define IS_RANGE(value) (fodiIsObjType(value, OBJ_RANGE))       // ObjRange
#define IS_STRING(value) (fodiIsObjType(value, OBJ_STRING))     // ObjString
...
#define CONST_STRING(vm, text) fodiNewStringLength((vm), (text), sizeof(text) - 1)
...
typedef struct sObjUpvalue
{
  ...
  // collected, but they are not first class Fodi objects.
  Obj obj;
...
typedef bool (*Primitive)(FodiVM *vm, Value *args);
...
typedef enum
{
  ...
  // initial fiber used by a call to `fodiCall()` or `fodiInterpret()`.
  FIBER_ROOT,
...
// [stackStart].
static inline void fodiAppendCallFrame(FodiVM *vm, ObjFiber *fiber,
                                       ObjClosure *closure, Value *stackStart, int returnReg)
{
  ...
  frame->rip = closure->fn->code.data;
...
static inline bool fodiHasError(const ObjFiber *fiber)
{
...
static inline bool fodiMapIsValidKey(Value arg);
...
// [needle].
uint32_t fodiStringFind(ObjString *haystack, ObjString *needle,
                        uint32_t startIndex);
...
static inline bool fodiStringEqualsCString(const ObjString *a,
...
//
// Unlike fodiGetClassInline in fodi_vm.h, this is not inlined. Inlining helps
...
static inline bool fodiValuesSame(Value a, Value b)
{
#if FODI_NAN_TAGGING
...
// [IS_BOOL].
static inline bool fodiIsBool(Value value)
{
#if FODI_NAN_TAGGING
...
static inline bool fodiIsObjType(Value value, ObjType type)
{
  return IS_OBJ(value) && AS_OBJ(value)->type == type;
}

// Converts the raw object pointer [obj] to a [Value].
static inline Value fodiObjectToValue(Obj *obj)
{
#if FODI_NAN_TAGGING
...
static inline double fodiValueToNum(Value value)
{
#if FODI_NAN_TAGGING
  return fodiDoubleFromBits(value);
#else
  return value.as.num;
#endif
}
...
static inline Value fodiNumToValue(double num)
{
#if FODI_NAN_TAGGING
  return fodiDoubleToBits(num);
#else
  Value value;
  value.type = VAL_NUM;
  value.as.num = num;
