...
#define DEBUG_TRACE_REG_INSTRUCTIONS()                                  \
  ...
  {                                                                    \
    ...
    wrenDumpRegStack(fiber, stackStart, baseIndex + fn->stackTop.data[(rip - fn->regCode.data)]);                                \
...
#define REG_DISPATCH()                                                           \
  ...
  {                                                                              \
    ...
    COUNT_OPCODE();                                                              \
    fiber->stackTop =                                                            \
      (stackStart - fiber->stack) + fn->stackTop.data[(rip - fn->regCode.data)]; \
...
#define REG_DISPATCH() goto loop
  ...
  REG_INTERPRET_LOOP
  {
    ...
    }
    {
      ...
      CASE_OP(NOT) : opperand = READ(GET_B(code));
      ...
      CASE_OP(NEG) : opperand = READ(GET_B(code));
      if (IS_CLASS(opperand) || IS_INSTANCE(opperand))
      {
        targetClass = wrenGetClassInline(vm, opperand);
        symbol = wrenSymbolTableFind(&vm->methodNames, "-", 1);
    ...
    unaryOverload:
      ...
      fiber->lastCallReg = baseIndex + GET_A(code);
      fiber->stack[fiber->stackTop] = opperand;

      STORE_FRAME();
      wrenCallFunction(vm, fiber, (ObjClosure *)method->as.closure, fiber->stack + fiber->stackTop, 1, baseIndex + GET_A(code));
      LOAD_FRAME();
      REG_DISPATCH();
    }

    {
      ...
      CASE_OP(EQ) : left = READ(GET_B(code));
      right = READ(GET_C(code));
      goto finishEQ;
      CASE_OP(EQK) : left = GET_K(code) == 0 ? READ(GET_B(code)) : fn->constants.data[GET_C(code)];
    ...
    finishEQ:
      ...
      CASE_OP(LT) : left = RKREAD(GET_B(code));
      right = RKREAD(GET_C(code));
      goto finishLT;
      CASE_OP(LTK) : left = GET_K(code) == 0 ? RKREAD(GET_B(code)) : fn->constants.data[GET_C(code)];
    ...
    finishLT:
      ...
      CASE_OP(LTE) : left = RKREAD(GET_B(code));
      right = RKREAD(GET_C(code));
      goto finishLTE;
      CASE_OP(LTEK) : left = GET_K(code) == 0 ? RKREAD(GET_B(code)) : fn->constants.data[GET_C(code)];
      right = GET_K(code) == 1 ? RKREAD(GET_B(code)) : fn->constants.data[GET_C(code)];
      goto finishLTE;
    ...
    finishLTE:
      ...
      CASE_OP(ADD) : left = READ(GET_B(code));
      right = READ(GET_C(code));
      goto finishADD;

      CASE_OP(ADDK) : left = GET_K(code) == 0 ? RKREAD(GET_B(code)) : fn->constants.data[GET_C(code)];
      right = GET_K(code) == 1 ? RKREAD(GET_B(code)) : fn->constants.data[GET_C(code)];
      goto finishADD;
    ...
    finishADD:
      ...
      CASE_OP(SUB) : left = READ(GET_B(code));
      right = READ(GET_C(code));
      goto finishSUB;
      CASE_OP(SUBK) : left = GET_K(code) == 0 ? RKREAD(GET_B(code)) : fn->constants.data[GET_C(code)];
      right = GET_K(code) == 1 ? RKREAD(GET_B(code)) : fn->constants.data[GET_C(code)];
      goto finishSUB;
    ...
    finishSUB:
      ...
      right = RKREAD(GET_C(code));
      goto finishMUL;
      CASE_OP(MULK) : left = GET_K(code) == 0 ? RKREAD(GET_B(code)) : fn->constants.data[GET_C(code)];
      right = GET_K(code) == 1 ? RKREAD(GET_B(code)) : fn->constants.data[GET_C(code)];
      goto finishMUL;
    ...
    finishMUL:
      ...
      right = RKREAD(GET_C(code));
      goto finishDIV;
      CASE_OP(DIVK) : left = GET_K(code) == 0 ? RKREAD(GET_B(code)) : fn->constants.data[GET_C(code)];
      right = GET_K(code) == 1 ? RKREAD(GET_B(code)) : fn->constants.data[GET_C(code)];
      goto finishDIV;
    ...
    int needed;
    checkOverload:
      baseIndex = stackStart - fiber->stack;
      fiber->lastCallReg = baseIndex + GET_A(code);

      needed = fiber->stackTop + method->as.closure->fn->maxSlots;
      wrenEnsureStack(vm, fiber, needed);

      fiber->stack[fiber->stackTop] = left;
      fiber->stack[fiber->stackTop + 1] = right;

      STORE_FRAME();
      wrenCallFunction(vm, fiber, (ObjClosure *)method->as.closure, fiber->stack + fiber->stackTop, 2, baseIndex + GET_A(code));
      LOAD_FRAME();
      REG_DISPATCH();
    ...
    comparisonOverload:
      ...
      {
        fiber->lastCallReg = fiber->stackTop;
      }
      ...
      STORE_FRAME();
      wrenCallFunction(vm, fiber, (ObjClosure *)method->as.closure, fiber->stack + fiber->stackTop, 2, fiber->lastCallReg);
      LOAD_FRAME();
      REG_DISPATCH();
    }
    }

    {
      ...
      iterateOverload:
        ...
        int needed = fiber->stackTop + method->as.closure->fn->maxSlots;
        wrenEnsureStack(vm, fiber, needed);

        fiber->lastCallReg = baseIndex + GET_A(code);

        fiber->stack[fiber->stackTop] = sequence;
        fiber->stack[fiber->stackTop + 1] = iterator;

        // wrenDumpRegStack(fiber, stackStart, stackTop + 1);
        STORE_FRAME();
        wrenCallFunction(vm, fiber, (ObjClosure *)method->as.closure, fiber->stack + fiber->stackTop, 2, baseIndex + GET_A(code));
        LOAD_FRAME();
        REG_DISPATCH();
...
WrenHandle *wrenMakeCallHandle(WrenVM *vm, const char *signature)
{
  ...
  wrenIntBufferFill(vm, &fn->stackTop, 0, 2);
